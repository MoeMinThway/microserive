...
public class AmountController {
    ...
    record RequestAccount(String email,double amount){}  //return pyan chin tr ko  custom yay
    ...
    public ResponseAccount withDraw(
       ...
       double amount= accountService
               .withDraw(requestAccount.email, requestAccount.amount);
    ...
    public   ResponseTransferAccount transfer(
            @RequestBody RequestTransferAccount account
    ){
       accountService.transfer(account.fromEmail, account.toEmail,account.amount);
        return new ResponseTransferAccount(String.format("%s transfer $%s amount to %s successfully",
                account.fromEmail,account.toEmail, account.amount));
    }
}
